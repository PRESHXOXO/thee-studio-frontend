...
export async function fetchBillingCatalog(client = supabase, now = new Date()) {
  ...
  return (plansResult.data || []).map(plan => {
    const versions = (versionsResult.data || [])
      .filter(version => version.plan_id === plan.id && new Date(version.effective_from).getTime() <= instant && (!version.effective_to || new Date(version.effective_to).getTime() > instant))
