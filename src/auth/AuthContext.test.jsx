...
function Probe() {
  ...
  return (
    <div>
      <span>{auth.loading ? 'loading' : auth.user?.id || 'signed-out'}</span>
      <span>{auth.error}</span>
      <button onClick={() => auth.signIn('owner@example.invalid', 'password').catch(() => {})}>login</button>
...
describe('AuthProvider', () => {
  it('restores an existing session', async () => {
    const client = fakeClient({ access_token: 'test-token', user: { id: 'owner-1' } });
  ...
  it('signs in with password authentication', async () => {
    ...
    client.auth.signInWithPassword.mockResolvedValue({ data: { session: { access_token: 'test-token', user: { id: 'owner-2' } }, user: { id: 'owner-2' } }, error: null });
    render(<AuthProvider client={client}><Probe /></AuthProvider>);
  ...
  it('signs out and clears local auth state', async () => {
    const client = fakeClient({ access_token: 'test-token', user: { id: 'owner-3' } });
    render(<AuthProvider client={client}><Probe /></AuthProvider>);
  ...
  it('handles an expired session event', async () => {
    const client = fakeClient({ access_token: 'test-token', user: { id: 'owner-4' } });
    render(<AuthProvider client={client}><Probe /></AuthProvider>);
