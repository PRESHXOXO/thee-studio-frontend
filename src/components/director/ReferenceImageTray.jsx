...
function nextSuggestedRole(currentRole, references, roleOptions) {
  const allowed = new Set(roleOptions.map(option => option.id));
  const used = new Set(references.map(reference => reference.role));
  const sequence = ROLE_SEQUENCE.filter(role => allowed.has(role));
  const start = Math.max(0, sequence.indexOf(currentRole) + 1);
  return sequence.slice(start).find(role => !used.has(role)) || sequence.find(role => !used.has(role)) || sequence[0] || 'outfit';
}
...
export function ReferenceImageTray({
  ...
  const safeDefaultRole = roleOptions.some(role => role.id === defaultRole) ? defaultRole : roleOptions[0]?.id || 'outfit';
  ...
  React.useEffect(() => {
    ...
    else if (!references.length) setNextRole(safeDefaultRole);
  }, [identityLocked, safeDefaultRole, references.length, nextRole, roleOptions]);

  // A selected Cast member owns Identity. Clean up any stale client Identity
  // card left behind by an older Director session instead of sending a second
  // identity authority to the provider.
  React.useEffect(() => {
    if (!identityLocked || !references.some(reference => reference.role === 'identity')) return;
    onChange(references.filter(reference => reference.role !== 'identity'));
  }, [identityLocked]); // intentionally reacts only when lock mode changes
  ...
  async function handleFiles(event) {
    ...
    try {
      ...
      const added = { id: makeReferenceId(), dataUrl, name: file.name, role: nextRole, pending: true, source: 'upload' };
      const nextReferences = [...references, added];
      onChange(nextReferences);
      setNextRole(nextSuggestedRole(nextRole, nextReferences, roleOptions));
      if (files.length > 1) setError('Add one image at a time so each reference keeps one clear job.');
    } catch (uploadError) { setError(uploadError.message || 'Could not read that image.'); }
    finally { setReading(false); }
  }

  function changeRole(id, role) {
    if (!roleOptions.some(option => option.id === role)) return;
    onChange(references.map(reference => reference.id === id ? { ...reference, role, pending: true } : reference));
  }
  function removeReference(id) { onChange(references.filter(reference => reference.id !== id)); }

  const addControls = <div style={{ display: 'flex', alignItems: compact ? 'stretch' : 'center', gap: compact ? 6 : 10, flexDirection: compact ? 'column' : 'row', flexWrap: compact ? 'nowrap' : 'wrap', flex: compact ? '0 0 156px' : 'initial', padding: compact ? 0 : '12px 14px', border: compact ? 'none' : '1px dashed var(--border-strong)', borderRadius: compact ? 0 : 'var(--radius-lg)', background: compact ? 'transparent' : 'rgba(255,254,252,0.34)' }}>
    <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" style={{ display: 'none' }} onChange={handleFiles} />
    <button type="button" onClick={() => inputRef.current?.click()} disabled={disabled || reading || !available} style={{ minHeight: compact ? 32 : 40, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: compact ? '0 10px' : '0 14px', borderRadius: 9, border: '1px solid var(--plum)', background: 'var(--plum)', color: 'var(--text-on-dark)', cursor: disabled || reading || !available ? 'not-allowed' : 'pointer', opacity: disabled || reading || !available ? 0.48 : 1, font: `600 ${compact ? 11 : 12}px/1 var(--font-ui)`, fontFamily: 'inherit' }}><Icon name="plus" size={compact ? 12 : 14} />{reading ? 'Reading…' : available ? 'Add image' : 'Reference limit reached'}</button>
    <select aria-label="Role for new references" value={nextRole} onChange={event => setNextRole(event.target.value)} disabled={disabled || !available} style={{ minHeight: compact ? 32 : 40, padding: compact ? '0 8px' : '0 11px', border: '1px solid var(--border)', borderRadius: 9, background: 'rgba(255,254,252,0.72)', color: 'var(--text-body)', font: `600 ${compact ? 11 : 12}px/1 var(--font-ui)`, fontFamily: 'inherit' }}>{roleOptions.map(role => <option key={role.id} value={role.id}>{role.label}</option>)}</select>
    {!compact && available > 0 && <span style={{ font: '500 0.72rem/1.35 var(--font-ui)', color: 'var(--text-faint)', marginLeft: 2 }}>{identityLocked ? 'Identity comes from the selected Cast · ' : ''}Choose one job, then add one image · {available} slot{available === 1 ? '' : 's'} open</span>}
  </div>;
