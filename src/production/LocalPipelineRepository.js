...
export class LocalPipelineRepository {
  ...
  async createSampleWorkspace() {
    if (this.state.projects.length) return null;
    let creator = this.state.creators[0];
    if (!creator) {
  ...
  async createProviderRun(input) {
    return this.add(this.state.runs, {
      ...
      progress: input.progress || 0, retry_of: input.retry_of || null,
      attempt: input.attempt || 1, cancel_requested_at: null,
  ...
  async getProviderRun(id) { return this.state.runs.find(item => item.id === id) || null; }
  async cancelProviderRun(id) {
    const run = await this.getProviderRun(id);
    if (!run || !['queued', 'running'].includes(run.status)) return run;
    Object.assign(run, { status: 'cancelled', cancel_requested_at: timestamp(), completed_at: timestamp() });
    await this.addRunEvent(id, 'cancelled', 'Generation cancelled by the user.');
    this.save();
    return run;
  }
  async recoverInterruptedRuns(maxAgeMs = 240000) {
    const cutoff = Date.now() - maxAgeMs;
    const interrupted = this.state.runs.filter(run => run.status === 'running' && new Date(run.started_at || run.created_at).getTime() < cutoff);
    for (const run of interrupted) {
      Object.assign(run, { status: 'failed', error_code: 'interrupted', error_message: 'Generation was interrupted and can be retried.', completed_at: timestamp() });
      await this.addRunEvent(run.id, 'recovered', 'Interrupted generation marked retryable.');
    }
    this.save();
  }
  async getUsageSummary() {
    return { ...this.state.usage, remaining: Math.max(this.state.usage.included - this.state.usage.used, 0) };
  }
  async reserveLocalCredits(units) {
    if (this.state.usage.used + units > this.state.usage.included) throw new Error('Generation credit limit reached.');
    this.state.usage.used += units;
    this.save();
  ...
  async getStillAsset(id) {
    ...
    if (!asset) throw new Error('Still asset not found.');
    return asset;
  }
  async getClip(id) {
