...
import { promptLabBuild, characterGenerate, castQuickShootPlain } from '../api/studio.js';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
        {opts.map(o => <Pill key={o} label={o} active={value === o} onClick={() => onChange(o)} />)}
      </div>
    </div>
  );
}
...
export function PromptLab({ onNav, campaignId = null, initialVision = '', initialSettings = null, creator = null }) {
  ...
  const [refImage, setRefImage] = React.useState(() => creator?.refImages?.[0] || creator?.image || null);
  ...
  const adapter = getAdapter(MANAGED_TARGET);
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      if (res.refusal) {
        ...
        setHistory(pushHistory({ rawInput, request, result: res }));
      }
    } catch (e) {
      setError(e.message || 'The prompt could not be built. Try again.');
    } finally {
      setBuilding(false);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (refImage) {
        ...
        const res = await castQuickShootPlain({
          positivePrompt: generationPrompt,
          negativePrompt: STANDING_NEGATIVES,
          batchSize: 1,
        });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 'var(--content-max)', margin: '0 auto' }}>
      <div>
        ...
        <p style={{ font: 'var(--text-lg)', color: 'var(--text-muted)', margin: 0, maxWidth: 600 }}>
          Describe the image in plain, messy words. Thee Studio turns it into a cinema-grade prompt, then generates it right here.
        </p>
      </div>
      ...
      {result && (
        <Card style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ font: 'var(--text-xs)', color: 'var(--text-faint)' }}>
            Adjust anything, then hit Rebuild prompt above. “Surprise me” lets Thee Studio choose.
          </div>
        </Card>
      )}
      ...
      {result && activePrompt && (
        <Card style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
            <div style={{ font: '600 0.88rem/1 var(--font-ui)', color: 'var(--text-strong)' }}>Engineered Prompt</div>
            <span style={{ font: 'var(--text-xs)', color: 'var(--text-faint)' }}>{activePrompt.trim().split(/\s+/).length} words</span>
          </div>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <GenerationProgress active={generating} identityLocked={!!refImage} batchSize={1} style={{ flex: 1 }} />
            </div>
            ...
            {genImages.length > 0 && (
              <>
                ...
                <div style={{ font: 'var(--text-xs)', color: 'var(--text-faint)' }}>Saved to your Library automatically.</div>
              </>
            )}
          </div>
        </Card>
              ...
              {history.map(entry => (
                <div key={entry.id} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '12px 16px', borderRadius: 'var(--radius-md)', background: 'var(--surface-raised)', border: '1px solid var(--border)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ font: 'var(--text-xs)', color: 'var(--text-faint)', marginBottom: 4 }}>{new Date(entry.savedAt).toLocaleString()}</div>
