...
export function SceneFlowV2({ campaignId = null, initialVision = '', initialSettings = null, creator = null }) {
  ...
  const acceptPhotoResult = (result, sceneData = scene) => {
    ...
    images.forEach(url => saveToLibrary(url, { source: 'scene_flow', prompt, campaign: campaignId || undefined, character, mediaType: 'photo', settings: { version: 2, workflow: 'talk', input: scenePrompt(sceneData), outputType, batchSize, scene: sceneData, referenceRoles: roleSummary }, memoryVersion: memory?.version }).catch(() => {}));
  };
  ...
  async function runGeneration(sceneData = scene) {
    ...
    try {
      if (outputType === 'video') {
        ...
        acceptPhotoResult(result, sceneData);
      }
    } catch (err) {
  ...
  return <div style={ROOT}>
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <GenerationProgress active={generating || pendingGeneration.renderStatus === 'still_processing'} identityLocked={identity.locked} batchSize={outputType === 'photo' ? batchSize : 1} engine={outputType === 'video' ? 'Managed Video' : 'OpenAI'} mode={outputType === 'video' ? 'video' : 'scene'} />
      <div ref={bottomRef} />
    </div>
