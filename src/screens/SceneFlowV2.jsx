...
function sceneSummary(scene) { return scene ? [scene.setting, scene.location, scene.wardrobe, scene.vibe].filter(Boolean).join(' · ') : ''; }
function scenePrompt(scene) { return scene?.full_prompt || [scene?.setting, scene?.wardrobe, scene?.location, scene?.vibe].filter(Boolean).join('. '); }
...
export function SceneFlowV2({ campaignId = null, initialVision = '', initialSettings = null, creator = null }) {
  ...
  React.useEffect(() => {
    ...
    setMessages([]); setHistory([]); setInput(initialVision || ''); setReferences([]); setScene(null); setThinking(false); setGenerating(false);
  ...
  const roleSummary = references.map(reference => reference.role);
  ...
  async function send(text) {
    ...
    try {
      const castContext = creator
        ...
        : '';
      const refsContext = pendingRefs.length ? `Attached visual roles: ${pendingRefs.map(reference => reference.role).join(', ')}.` : '';
      const backendMessage = [castContext, message || refsContext, message && refsContext].filter(Boolean).join('\n\n');
      const response = await sceneFlowChat({ messagesJson: JSON.stringify(history), userMessage: backendMessage, referenceImages: pendingRefs });
      ...
      const nextScene = sceneData ? { ...(scene || {}), ...sceneData, content_type: outputType } : scene;
  ...
  async function runGeneration(sceneData = scene) {
    if (!sceneData || generating || identity.warning) return;
    setGenerating(true);
    ...
    try {
      if (outputType === 'video') {
        const keyframe = await generateDirectorPhoto({ creator, prompt, references, batchSize: 1, imageSize: sceneData.imageSize || sceneData.aspect || 'Vertical 9:16', requestKey: `${crypto.randomUUID()}-keyframe` });
        ...
        const result = await generateDirectorPhoto({ creator, prompt, references, batchSize, imageSize: sceneData.imageSize || sceneData.aspect || sceneData.aspect_ratio || 'Vertical 9:16' });
        const images = result.images || [];
        if (images.length !== batchSize) throw new Error(`Director requested ${batchSize} image${batchSize === 1 ? '' : 's'} but received ${images.length}. The incomplete batch was not silently accepted.`);
        setMessages(current => [...current, { role: 'assistant', text: 'Your scene is ready. ✨', images }]);
        images.forEach(url => saveToLibrary(url, { source: 'scene_flow', prompt, campaign: campaignId || undefined, character: creator?.id, mediaType: 'photo', settings: { version: 2, workflow: 'talk', input: scenePrompt(sceneData), outputType, batchSize, scene: sceneData, referenceRoles: roleSummary }, memoryVersion: memory?.version }).catch(() => {}));
      }
    } catch (err) { setMessages(current => [...current, { role: 'assistant', text: `⚠️ Generation failed: ${err.message}` }]); }
    finally { setGenerating(false); }
  }

  function reset() { setMessages([]); setHistory([]); setInput(initialVision || ''); setReferences([]); setScene(null); setThinking(false); setGenerating(false); }
  ...
  return <div style={ROOT}>
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <GenerationProgress active={generating} identityLocked={identity.locked} batchSize={outputType === 'photo' ? batchSize : 1} engine={outputType === 'video' ? 'Managed Video' : 'OpenAI'} mode={outputType === 'video' ? 'video' : 'scene'} />
      <div ref={bottomRef} />
    </div>
