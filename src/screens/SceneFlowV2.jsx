...
export function SceneFlowV2({ campaignId = null, initialVision = '', initialSettings = null, creator = null, recoveryEnabled = true }) {
  ...
  const renderLocked = generating || pendingGeneration.renderStatus === 'still_processing';

  async function send(text) {
    const message = (text ?? input).trim();
    const pendingRefs = references.filter(reference => reference.pending);
    if ((!message && !pendingRefs.length) || thinking || renderLocked) return;
  ...
  async function runGeneration(sceneData = scene) {
    if (!sceneData || renderLocked || identity.warning) return;
    let validatedScene;
  ...
  return <div style={ROOT}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid var(--border)', background: 'var(--surface-inset)' }}><div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--accent)', color: '#fff', display: 'grid', placeItems: 'center', font: '700 14px/1 var(--font-ui)' }}>S</div><div style={{ flex: 1 }}><div style={{ font: '600 15px/1 var(--font-ui)', color: 'var(--text-strong)' }}>Scene Flow</div><div style={SMALL}>Brainstorm freely. Rendering starts only from the explicit Generate button.</div></div>{messages.length > 0 && <button type="button" onClick={reset} disabled={thinking || renderLocked} style={{ padding: '7px 11px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', font: '500 12px/1 var(--font-ui)' }}>New chat</button>}</div>
    ...
    <div aria-label="Scene Flow conversation" style={CONVERSATION}>
      ...
      {scene && <div aria-label="Scene Flow shot board" style={{ display: 'grid', gap: 10 }}>
        ...
        {scene.shots.map(shot => <article key={shot.id} data-shot-id={shot.id} style={{ padding: 12, border: '1px solid var(--border)', borderRadius: 12, background: 'var(--surface-inset)', display: 'grid', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}><strong style={{ flex: 1, font: '600 12px/1 var(--font-ui)' }}>Shot {shot.index}</strong><button type="button" aria-label={`Move shot ${shot.index} up`} onClick={() => moveShot(shot.id, 'up')} disabled={shot.index === 1 || thinking || renderLocked}>↑</button><button type="button" aria-label={`Move shot ${shot.index} down`} onClick={() => moveShot(shot.id, 'down')} disabled={shot.index === scene.shots.length || thinking || renderLocked}>↓</button><button type="button" aria-label={`Delete shot ${shot.index}`} onClick={() => deleteShot(shot.id)} disabled={scene.shots.length === 1 || thinking || renderLocked}>×</button></div>
          <input aria-label={`Shot ${shot.index} action`} value={shot.action} onChange={event => editShot(shot, { action: event.target.value })} disabled={thinking || renderLocked} placeholder="Action or moment" style={{ padding: 8, border: '1px solid var(--border)', borderRadius: 8, font: 'var(--text-xs)' }} />
          ...
          <button type="button" onClick={() => addShot(shot.id)} disabled={thinking || renderLocked || scene.shots.length >= 12} style={{ justifySelf: 'start' }}>+ Add after</button>
        </article>)}
      </div>}
    ...
    <div style={{ borderTop: '1px solid var(--border)', background: 'var(--surface-inset)', padding: 14, display: 'flex', flexDirection: 'column', gap: 11 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}><textarea value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder="Describe the sequence or revise one shot. Rendering starts only from Generate." rows={1} disabled={thinking || renderLocked} style={{ flex: 1, minHeight: 42, maxHeight: 110, resize: 'vertical', padding: '10px 12px', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', background: 'var(--surface-card)', color: 'var(--text-body)', font: 'var(--text-sm)', fontFamily: 'inherit' }} /><button type="button" onClick={() => send()} disabled={thinking || renderLocked || !canSend} title="Send" style={{ width: 42, height: 42, borderRadius: 'var(--radius-lg)', border: 0, background: 'var(--accent)', color: '#fff', display: 'grid', placeItems: 'center', cursor: 'pointer', opacity: thinking || renderLocked || !canSend ? 0.45 : 1 }}><Icon name="arrow-up" size={16} /></button></div>
    </div>
  </div>;
}
